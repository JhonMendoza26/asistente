...
export class ProspectosSeguimientoComponent implements OnInit {
  ...
  constructor(
    ...
    private asistenteService: AsistenteVirtualService
  ) {}
  ...
  private obtenerDescripcionEtapas(etapaAnterior: number, etapaActual: number, etapaSiguiente: number): void {
    this.descripcionEtapas = this.asistenteService.obtenerDescripcionEtapaByIDs(
      etapaAnterior,
      etapaActual,
      etapaSiguiente
    );
  ...
  public desbloquearEtapas(): void {
    ...
    this.asistenteService.updateContacto(this.infoContacto.contacto?.idContacto || 0, contactoActualizado);
    this.obtenerInfoContacto();
    this.obtenerDescripcionEtapas(
      AsistenteVirtualConstants.CODE_ETAPA_ANTERIOR,
      AsistenteVirtualConstants.CODE_ETAPA_ACTUAL,
      AsistenteVirtualConstants.CODE_ETAPA_SIGUIENTE
    );
  }
